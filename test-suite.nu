...
(run_test "DateTime Creation" {
    let $date = dn new "System.DateTime" --args [2023, 12, 25]
    if ($date | describe) == "custom" { "SUCCESS" } else { error make { msg: "Expected custom type" } }
}),
...
(run_test "String Length Property" {
    let $str = "Hello World"
    let $length = $str | dn call "get_Length"
...
(run_test "List Creation and Operations" {
    ...
    if $count == 2 { "SUCCESS" } else { error make { msg: $"Expected 2, got ($count)" } }
}),
...
# Test 8: DateTime Methods
(run_test "DateTime AddDays Method" {
    let $date = dn new "System.DateTime" --args [2023, 1, 1]
    let $new_date = $date | dn call "AddDays" 10
    let $day = $new_date | dn get "Day"
    if $day == 11 { "SUCCESS" } else { error make { msg: $"Expected day 11, got ($day)" } }
}),

# Test 9: String Builder
(run_test "StringBuilder Operations" {
    let $sb = dn new "System.Text.StringBuilder"
    $sb | dn call "Append" "Hello"
    $sb | dn call "Append" " "
    $sb | dn call "Append" "World"
    let $result = $sb | dn call "ToString"
    if $result == "Hello World" { "SUCCESS" } else { error make { msg: $"Expected 'Hello World', got '($result)'" } }
}),
...
let $original = dn new "System.DateTime" --args [2023, 12, 25, 15, 30, 45]
...
if $failed == 0 {
    ...
    print $"⚠️  ($failed) test(s) failed. Please review the failures above."
}
